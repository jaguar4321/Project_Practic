...
class LessonVisitAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        try:
            # Викликаємо метод save моделі напряму
            obj.save()
            # Якщо збереження успішне, додаємо повідомлення про успіх
            messages.success(request, f'Lesson_visit "{obj}" було додано успішно.')
        except IntegrityError as e:
            # Додаємо повідомлення про помилку
            messages.error(request, str(e))
            return  # Завершуємо метод без виклику super()
